...
当前开发候选新增 **Cursor 客户端 3.20.17** 实验接入：在「设置 → Agent」启用 Cursor，并为 LoopFwd 开启 macOS 辅助功能权限。沿用账号登录，在本机读取已渲染 Agent 面板中选中的会话状态与当前步骤；隐藏会话、审批及成功完成不做推断，返回仅打开 Cursor。此界面观察不发送到可选的 Session Hub。
