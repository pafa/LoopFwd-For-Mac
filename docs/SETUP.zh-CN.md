...
从 [0.1.2 发布页](https://github.com/pafa/LoopFwd-For-Mac/releases/tag/v0.1.2) 下载 ZIP 及同名 SHA-256 文件，在下载目录校验：
...
要获得额度数据源、跳转错误提示和设置状态修复，请按上述步骤将 0.1.1 替换为 0.1.2。
原有设置会保留，目前没有自动更新器；原 0.1.1 下载包保持不变。
...
设置状态区分已禁用、已安装但未验证、仅有进程数据、读取失败，以及成功读取但无任务。
每行提供对应的 Agent、集成或诊断入口，不会自动安装观察器。
...
额度与任务发现共用被观察的 Codex 数据目录，包括自定义 CLI 目录和 Agent 页显式选择的目录。
自动检测到多个账户目录时需要指定一个，不合并不同账户的限额。窗口重置时间过后会隐藏，
等待 Codex 再次报告，不推测为 100% 余量；有界读取会考虑跨月、跨年的最近修改文件及已知活动会话。

跳转失败会区分已知的终端自动化权限拒绝、超时、目标关闭和辅助程序错误，并提供刷新或设置入口；
不会改为向其他窗口输入。声音导入若恢复失败，会显示保留的备份位置，请保留副本后再重试。
通用页的空闲清理时间适用于 Claude CLI、Codex CLI、OpenCode TUI/桌面版和 Qwen 记录，
不是全部接入的统一过期时间；Idle 始终不占据主岛。
...
**从 0.1.1 起，Terminal.app 仅支持监控和返回**：已移除全局键盘输入及其辅助功能授权入口。
